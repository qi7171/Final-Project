...
OscP5 oscP5;
Serial arduinoPort;
String portName = "/dev/cu.usbmodem11201"; // 根据你的Arduino连接端口进行更改
...
int graphicDisplayDuration = 3000;  // 图形展示时间
...
void generateGraphicsAndImages() {
  ...
  float d = grid * 0.6;
  int x = margin + currentCol * (grid + 50);
  int y = margin + currentRow * (grid + 50);

  if (millis() - lastActionTime < graphicDisplayDuration) {
    // 显示图形
    int colArrayNum = (int) random(7);
    stroke(colArray[colArrayNum]);
    DisplayObject newObject = new DisplayObject(x, y, d, colArray[colArrayNum], true);
    displayObjects.add(newObject);
  } else {
    // 在生成图像前移除图形
    Iterator<DisplayObject> iterator = displayObjects.iterator();
...
void resetAll() {
  ...
  personDetected = false; // 如果你想保留检测到的人的状态，可以删除这行代码
}
